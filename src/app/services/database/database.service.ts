...
export interface Dev {    //TODO: Change
  id: number,
  name: string,
  skills: any[],
  img: string
...
// All the calls to the database will take place in this service
@Injectable({
  providedIn: 'root'
})
export class DatabaseService {
  ...
  constructor(private plt: Platform, private sqlitePorter: SQLitePorter, private sqlite: SQLite, private http: HttpClient) {
    this.plt.ready().then(() => {
      this.sqlite.create({
        name: 'developers.db',    //TODO: Change
        location: 'default'
      })
  ...
  seedDatabase() {
    this.http.get('assets/populate.sql', { responseType: 'text'})
    .subscribe(sql => {
      this.sqlitePorter.importSqlToDb(this.database, sql)
        .then(_ => {
          this.loadDevelopers();    //TODO: Change
          this.loadProducts();      //TODO: Change
  ...
  getDevs(): Observable<Dev[]> {                //TODO: Change
    return this.developers.asObservable();
  }
 
  getProducts(): Observable<any[]> {            //TODO: Change
    return this.products.asObservable();
  }

  // CRUD OPERATIONS
  loadDevelopers() {
    return this.database.executeSql('SELECT * FROM developer', []).then(data => { //TODO: Change
      let developers: Dev[] = [];
